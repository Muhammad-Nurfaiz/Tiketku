...
export default function EventAdminEvents() {
  ...
  const [form, setForm] = useState({ name: '', category: '', region: '', venue: '', start: '', end: '', desc: '' });

  const columns = [
    { key: 'name', label: 'Nama Event' },
    { key: 'venue', label: 'Venue' },
    { key: 'start', label: 'Tanggal Mulai' },
    { key: 'end', label: 'Tanggal Selesai' },
    { key: 'status', label: 'Status' },
    { key: 'totalTickets', label: 'Total Tiket' },
    { key: 'sold', label: 'Tiket Terjual' },
  ];
  ...
  const filtered = data.filter((d) => {
    ...
    const matchesSearch = !q || [d.name, d.venue, d.start, d.end].join(' ').toLowerCase().includes(q);
    const matchesStatus =
      statusFilter === 'All' ? true : normalizeStatus(d.status) === statusFilter;
  ...
  function handleAdd(e) {
    e.preventDefault();
    const item = { id: data.length + 1, ...form, totalTickets: 100, sold: 0 };
    setData([item, ...data]);
    setOpen(false);
    setForm({ name: '', category: '', region: '', venue: '', start: '', end: '', desc: '' });
    setPage(1);
  }
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm divide-y">
            ...
            <tbody className="divide-y">
              {pageData.length === 0 ? (
                <tr>
                  <td colSpan={columns.length + 1} className="px-3 py-6 text-center text-slate-500">No events found.</td>
                </tr>
              ) : (
                pageData.map((row) => (
                  <tr key={row.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-3 py-3">{row.totalTickets}</td>
                    <td className="px-3 py-3">{row.sold}</td>
                    <td className="px-3 py-3 text-right">
                      <div className="inline-flex items-center gap-2">
                        <button
                          className="p-2 rounded hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-slate-200"
                          aria-label="Edit"
                        >
                          <svg className="w-4 h-4 text-slate-600" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5">
                            <path d="M4 13v3h3l9-9-3-3L4 13z" strokeLinecap="round" strokeLinejoin="round" />
                          </svg>
                        </button>
                        <button
                          className="p-2 rounded hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-slate-200"
                          aria-label="Delete"
                        >
                          <svg className="w-4 h-4 text-red-600" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5">
                            <path d="M6 7l1 10a2 2 0 0 0 2 2h2a2 2 0 0 0 2-2l1-10" strokeLinecap="round" strokeLinejoin="round" />
                            <path d="M9 4V3a1 1 0 0 1 1-1h0a1 1 0 0 1 1 1v1" strokeLinecap="round" strokeLinejoin="round" />
                            <path d="M4 7h12" strokeLinecap="round" strokeLinejoin="round" />
                          </svg>
                        </button>
                      </div>
                    </td>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <form onSubmit={handleAdd} className="space-y-4">
            <div>
              <label className="block text-sm mb-1">Nama Event</label>
            ...
            <div>
              <label className="block text-sm mb-1">Wilayah</label>
